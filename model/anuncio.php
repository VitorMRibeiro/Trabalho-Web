...
class anuncio
{
    ...
    public function get_anuncios($categoria = ""){
        ...
        if($categoria == ""){
            ...
            $result = $this->mysqli->query("SELECT * FROM anuncios WHERE categoria = '$categoria'");
            if($result){
                while( $row = $result->fetch_assoc()){
                    array_push($return, $row);
                }
    ...
    public function get_anuncios_usuario($login){
        $return = [];
        $result = $this->mysqli->query("SELECT anuncios.id, anuncios.nome, anuncios.categoria, anuncios.descricao FROM anuncios JOIN usuario_anuncio ON anuncios.id = usuario_anuncio.anuncio_id WHERE usuario_anuncio.usuario_login = '$login'");
        if($result){
            while( $row = $result->fetch_assoc()){
                array_push($return, $row);
            }
    ...
    public function salvar_anuncio($usuario_login, $nome, $categoria, $descricao){
        ...
        if(!$this->mysqli->query("INSERT INTO anuncios (nome, categoria, descricao) VALUES ('$nome', '$categoria', '$descricao')")){ 
        ...
        // isere na tabela usuario_anuncio
        if(!$this->mysqli->query("INSERT INTO usuario_anuncio (usuario_login, anuncio_id) VALUES ('$usuario_login', " . $id . ")")){ 
            echo "Falha ao adicionar anuncio: (" . $this->mysqli->errno . ") " . $this->mysqli->error;
            return false;
        }
        return $id;
    ...
    function __construct(){
        ...
        $this->mysqli->query("CREATE TABLE IF NOT EXISTS anuncios (id INTEGER AUTO_INCREMENT PRIMARY KEY, nome VARCHAR(20), categoria VARCHAR(20), descricao VARCHAR(200))");
        $this->mysqli->query("CREATE TABLE IF NOT EXISTS usuario_anuncio (usuario_login VARCHAR(20), anuncio_id INTEGER)");
